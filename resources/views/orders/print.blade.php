<style type="text/css">
	@page { margin: 2px; }
	body { 
		margin: 2px;
		margin-top: 1em;
	}
	.f11
	{
		font-size: 11px;
	}
	.f13
	{
		font-size: 13px;
	}
	.f15
	{
		font-size: 15px;
	}
	.f20
	{
		font-size: 20px;
	}
	@page { size: 100mm 150mm lands; }
</style> 

<body>
	<table width="100%" cellpadding="5" borderpadding="0" style="border: 1px solid #000; font-family: sans-serif;">
		<tr>
			<td style="width: 10%">
				<img src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->size(75)->errorCorrection('H')
	                        ->generate($order->order_number)) !!} ">
			</td>
			<td style="line-height: 20px">
				<strong class="f20">LOKALDATPH</strong><br>
				<strong class="f15">{{$order->order_number}}</strong><br>
				<strong class="f15">Delivery Receipt</strong>
			</td>
		</tr>
		<tr>
			<td colspan="2" style="padding:5px;padding-top:10px;padding-bottom: 10px;">
				<table width="100%" cellpadding="0" style="padding:3px;border:solid 1px #000000">
					
					<tr>
						<td colspan="2">
							<img style="width: 15px" src="data:image/png;base64,">
							<strong class="f13">Receiver</strong>
						</td>
					</tr>
					<tr>
						<td colspan="2" class="f13">To : {{$order->userFullName}}</td>
					</tr>
					<tr>
						<td colspan="2" class="f13">
							{{strtoupper($order->userShippingAddress.', '.$order->userBarangay.', '.$order->userCityMunicipality.', '.$order->userProvince)}}
						</td>
					</tr>
					<tr>
						<td colspan="2" class="f13">
							{{$order->userMobile}}
						</td>
					</tr>
					<tr>
						<td colspan="2" class="f11">&nbsp;</td>
					</tr>
					<tr>
						<td colspan="2">
							<img style="width: 15px" src="data:image/png;base64,">
							<strong class="f13">Shipper</strong>
						</td>
						
					</tr>
					<tr>
						<td colspan="2" class="f13">FROM : {{$sellers->name}}</td>
					</tr>
					<tr>
						<td colspan="2" class="f13">
							{{strtoupper($sellers->street_address.', '.$sellers->brgyDesc.', '.$sellers->citymunDesc.', '.$sellers->provDesc)}}
						</td>
					</tr>
					<tr>
						<td class="f13">Items :</td>
						<td style="text-align: right;" class="f13">Shirt(s)</td>
					</tr>
					
					<tr>
						<td class="f13">Mode :</td>
						<td style="text-align: right" class="f13"><strong>{{strtoupper($order->payment_method)}}</strong></td>
					</tr>
					<tr>
						<td colspan="2">
							<span class="f13">Payable Amount :</span>
							<span style="float: right;font-weight: bold;" class="f20">PHP&nbsp;{{number_format($order->seller_total, 2)}}</span>
						</td>
					</tr>
					<tr>
						<td colspan="2" class="f11">&nbsp;</td>
					</tr>
				</table>
				
			</td>
		</tr>
		<tr>
			<td colspan="2" style="font-weight: bold" class="f13">
				For returns, send us an email to:<br>
				support@lokaldatph.com
			</td>
		</tr>
		<tr>
			<td colspan="2"class="f11">&nbsp;</td>
		</tr>
		<tr>
			<td colspan="2" style="text-align: center;font-weight: bold" class="f13">
				www.lokaldatph.com
			</td>
		</tr>
		<tr>
			<td colspan="2" style="text-align: center;color:gray" class="f13">
				Print
			</td>
		</tr>
		<tr>
			<td colspan="2" class="f11">&nbsp;</td>
		</tr>
	</table>
</body>
